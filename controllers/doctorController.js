...
const createDoctor =async (req,res)=>{
    const {name,specilization} = req.body;
    const emptyfiels= []
    if(!name)
    {
    ...
    try {
        const doctor = new Doctor({name,specilization});
        doctor.save();
        res.status(200).json(doctor)
...
module.exports={getDoctor,createDoctor,getAllDoctors,updateDoctor,deleteDoctor}